...
<head>
    ...
    <title>@yield('page-title') |
        {{ !empty($admin_settings['title_text']) ? $admin_settings['title_text'] : config('app.name', 'Stockology') }}
    </title>
    ...
    <link rel="icon"
        href="{{ (!empty($admin_settings['favicon']) && check_file($admin_settings['favicon'])) ? get_file($admin_settings['favicon']) : get_file('uploads/logo/favicon.png')}}"
        type="image/x-icon" />
    <!-- CSS Libraries -->
...
<body class="{{ $themeColor }}">
    <div class="custom-login">
        <div class="login-bg-img">
            ...
            <img src="{{ isset($admin_settings['color_flag']) && $admin_settings['color_flag'] == 'false' ? asset('images/' . $color . '.svg') : asset('images/theme-1.svg')  }}"
                class="login-bg-1">
                        ...
                        <div class="navbar-brand">
                            <a class="navbar-brand" href="{{ url('/') }}">
                                <img src="{{ get_file(sidebar_logo()) }}{{'?' . time()}}"
                                    alt="{{ config('app.name', 'Stockology') }}"
