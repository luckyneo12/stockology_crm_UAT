<header
    ...
    @if(Request::routeIs('leads.index') || Request::routeIs('leads.list') || Request::routeIs('whatsapp.chats') || Request::routeIs('leads.show') || Request::routeIs('leads.details.show'))
        @php
            $headerUser = Auth::user();
            $wsId = getActiveWorkSpace();
            $availableWorkspaces = collect(getWorkspace())->map(fn($w) => [
                'id' => (string)$w->id,
                'name' => $w->name,
                'created_by' => (string)$w->created_by,
                'is_disable' => (int)$w->is_disable,
                'change_url' => route('workspace.change', $w->id),
                'edit_url' => route('workspace.edit', $w->id)
            ])->values();

            $langList = [];
            foreach(languages() as $k => $lang) {
                $langList[] = ['value' => $k, 'label' => Str::ucfirst($lang), 'change_url' => route('lang.change', $k)];
            }

            // Get Pipeline Selector Info (Leads pages only)
            ...
            if (isset($pipeline)) {
                ...
                foreach($pipelines as $k => $value) {
                    $pId = is_object($value) ? $value->id : $k;
            ...
            $userSettings = getCompanyAllSetting($headerUser->id, $headerUser->workspace_id);
            $availableApisList = [];
            for($i=1; $i<=2; $i++) {
                if(!empty($userSettings['user_api_'.$i.'_url_'.$headerUser->id])) {
                    $availableApisList[] = ['value' => 'user_'.$i, 'label' => $userSettings['user_api_'.$i.'_name_'.$headerUser->id] ?: 'User API '.$i];
                }
            }
            if(empty($availableApisList) && module_is_active('Hrm', $headerUser->workspace_id)) {
                $employee = \Workdo\Hrm\Entities\Employee::where('user_id', $headerUser->id)->first();
                if($employee && $employee->department_id) {
                    for($i=1; $i<=2; $i++) {
                        if(!empty($userSettings['dept_api_'.$i.'_url_'.$employee->department_id])) {
                            $availableApisList[] = ['value' => 'dept_'.$i, 'label' => $userSettings['dept_api_'.$i.'_name_'.$employee->department_id] ?: 'Dept API '.$i];
                        }
                    }
                }
            }
            if(empty($availableApisList)) {
                for($i=1; $i<=3; $i++) {
                    if(!empty($userSettings['global_calling_api_'.$i.'_url'])) {
                        $availableApisList[] = ['value' => 'global_'.$i, 'label' => $userSettings['global_calling_api_'.$i.'_name'] ?: 'Global API '.$i];
                    }
                }
            }
            $activeExtIdx = $headerUser->active_extension == 2 ? 2 : 1;
            $mappedApiId = $userSettings['user_ext_'.$activeExtIdx.'_api_id_'.$headerUser->id] ?? '';
            $activeApi = null;
            foreach($availableApisList as $api) {
                if((string)$api['value'] === (string)$mappedApiId) {
                    $activeApi = $api;
                    break;
                }
            }
        ...
        <div
            ...
            data-user="{{ json_encode([
                'id' => (string)$headerUser->id,
                'name' => $headerUser->name,
                'avatar' => !empty($headerUser->avatar) ? (check_file($headerUser->avatar) ? get_file($headerUser->avatar) : '') : '',
                'extension_1' => (string)$headerUser->extension_1,
                'extension_2' => (string)$headerUser->extension_2,
                'active_extension' => (int)$headerUser->active_extension,
                'profile_url' => route('profile'),
                'logout_url' => route('logout')
            ]) }}"
            data-active-workspace-name="{{ Auth::user()->ActiveWorkspaceName() }}"
            data-active-workspace-id="{{ (string)$wsId }}"
            data-workspaces="{{ json_encode($availableWorkspaces) }}"
            data-current-user-role="{{ Auth::user()->roles->first()?->name }}"
            data-languages="{{ json_encode($langList) }}"
            data-active-language="{{ getActiveLanguage() }}"
            data-notification-count="{{ (int)$notificationCount }}"
            data-pipeline-id="{{ $selectedPipeline ? $selectedPipeline['id'] : '' }}"
            data-pipeline-name="{{ $selectedPipeline ? $selectedPipeline['name'] : '' }}"
            data-pipelines="{{ json_encode($pipelineOptionsList) }}"
            data-available-apis="{{ json_encode($availableApisList) }}"
            data-active-api-id="{{ (string)$mappedApiId }}"
            data-active-api-name="{{ $activeApi ? $activeApi['label'] : '' }}"
            data-csrf="{{ csrf_token() }}"
            data-route-is-leads="{{ (Request::routeIs('leads.index') || Request::routeIs('leads.list')) ? '1' : '0' }}"
            data-route-name="{{ Route::currentRouteName() }}"
            data-can-create-workspace="{{ Auth::user()->isAbleTo('workspace create') ? '1' : '0' }}"
            data-can-manage-workspace="{{ Auth::user()->isAbleTo('workspace manage') ? '1' : '0' }}"
            data-can-create-lead="{{ Auth::user()->isAbleTo('lead create') ? '1' : '0' }}"
            data-can-import-lead="{{ Auth::user()->isAbleTo('lead import') ? '1' : '0' }}"
            data-create-workspace-url="{{ route('workspace.create') }}"
            data-create-lead-url="{{ route('leads.create') }}"
            data-import-lead-url="{{ route('leads.bulk.import') }}"
            data-leads-list-url="{{ route('leads.list') }}"
            data-leads-board-url="{{ route('leads.index') }}"
            data-impersonating="{{ method_exists(Auth::user(), 'isImpersonating') && Auth::user()->isImpersonating() ? '1' : '0' }}"
            data-exit-company-url="{{ route('exit.company') }}"
        ></div>
