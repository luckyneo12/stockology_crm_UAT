...
class EmployeeController extends Controller
{
    ...
    public function create()
    {
        if (Auth::user()->isAbleTo('employee create')) {
            ...
            // Fetch employees for Reporting To dropdown
            $managers = Employee::where('created_by', creatorId())->where('workspace', getActiveWorkSpace())->get()->pluck('name', 'id');
            // Adding '0' or '' as option for "No Manager" is handled in blade usually, or we can prepend here.

            return view('hrm::employee.create', compact('employees', 'employeesId', 'departments', 'designations', 'documents', 'branches', 'role', 'customFields', 'location_type', 'managers'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
        ...
        if (Auth::user()->isAbleTo('employee create')) {
            ...
            $employee = Employee::create(
                [
                    ...
                    'parent_id' => !empty($request['parent_id']) ? $request['parent_id'] : null, // Save Reporting Manager
                ]
            );
    ...
    public function edit($id)
    {
        ...
        if (Auth::user()->isAbleTo('employee edit')) {
            ...
            // Fetch managers (exclude self if needed, though filtered by view usually, here we just get all)
             $managers = Employee::where('created_by', creatorId())->where('workspace', getActiveWorkSpace())->where('id', '!=', $id)->get()->pluck('name', 'id');

            return view('hrm::employee.edit', compact('employee', 'user', 'employeesId', 'branches', 'departments', 'designations', 'document_types', 'customFields', 'location_type', 'managers'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
