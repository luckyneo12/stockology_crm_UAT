                ...
                <div class="col-md-3">
                    <div class="card sticky-top" style="top:30px">
                        <div class="list-group list-group-flush" id="useradd-sidenav">
                            ...
                            @if (!Auth::user()->hasRole('super admin') || in_array('Client View Tasks', $permission))
                            ...
                            @if (!Auth::user()->hasRole('client'))
                                <a class="list-group-item list-group-item-action border-0"
                                    href="#reminders">{{ __('Reminders') }}
                                    <div class="float-end"><i class="ti ti-chevron-right"></i></div>
                                </a>
                            @endif
                            @if (!Auth::user()->hasRole('super admin') || in_array('Client View Products', $permission))
                                <a class="list-group-item list-group-item-action border-0"
                                    href="#products-users">{{ __('Users | Products') }}
                                    <div class="float-end"><i class="ti ti-chevron-right"></i></div>
                                </a>
                            @endif
                            @if (!Auth::user()->hasRole('super admin') || in_array('Client View Sources', $permission))
                                <a class="list-group-item list-group-item-action border-0"
                            ...
                            @if (!Auth::user()->hasRole('super admin') || in_array('Client View Files', $permission))
                                <a class="list-group-item list-group-item-action border-0"
                                    href="#files">{{ __('Files') }}
                                    <div class="float-end"><i class="ti ti-chevron-right"></i></div>
                                </a>
                            ...
                            <div class="col-12">
                                @if($overdueTasksCount > 0)
                                    <div class="alert alert-danger alert-dismissible fade show d-flex align-items-center mb-2" role="alert">
                                        ...
                                        <div>
                                            <strong>{{ __('Overdue Tasks!') }}</strong> {{ __('You have') }} {{ $overdueTasksCount }} {{ __('overdue task(s) for this deal.') }}
                                                                        ...
                                                                        <div
                                                                            ...
                                                                            @permission('deal task edit')
                                                                                <input type="checkbox"
                                                                                    ...
                                                                                    role="switch"
                                                                                    id="task_{{ $task->id }}"
                                                                                    @if ($task->status == 'done') checked="checked" @endif type="checkbox" value="{{ $task->status }}" data-url="{{ route('deals.tasks.update_status', [$deal->id, $task->id]) }}"/>
                                                                            @endpermission
                                                                            <label for="task_{{ $task->id }}" class="custom-control-label ml-4 @if ($task->status == 'done') strike @endif">
                                                                                <h6 class="media-title text-sm form-check-label">
                                                                                    {{ $task->name }}
                                                                                    <div class="badge p-2 px-3 bg-{{ $task->status == 'done' ? 'success' : ($task->status == 'overdue' ? 'danger' : 'warning') }} mb-1">
                                                                                        {{ __(Workdo\Lead\Entities\DealTask::$status[$task->status]) }}
                                                                                    </div>
                                                                                </h6>
                                                                                <div class="text-xs text-muted">
                                                                                    {{ __(Workdo\Lead\Entities\DealTask::$priorities[$task->priority]) }} - 
                                                                                    <span class="text-primary">{{ company_datetime_formate($task->date . ' ' . $task->time) }}</span>
                                                                                    <br>
                                                                                    <small>{{ __('Assigned to') }}: {{ $task->user->name ?? '-' }}</small>
                                                                                </div>
                                                                                </label>
                                                                            </div>
                                                                        </td>
                            ...
                            <div class="col-md-12">
                                <div class="card table-card">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center">
                                            ...
                                            <div class="float-end">
                                                <a class="btn btn-sm btn-primary float-end "
                                                    data-bs-toggle="tooltip" data-bs-placement="top"
                                                    title="{{ __('Create') }}"
                                                    data-url="{{ route('deals.reminders.create', $deal->id) }}"
                                    ...
                                    <div class="card-body pt-0 table-border-style bg-none" style ="height:300px;overflow: auto;">
                                        <div class="">
                                            <table class="table align-items-center mb-0" id="reminders">
                                                <thead>
                                                    <tr>
                                                        <th>{{__('Title')}}</th>
                                                        <th>{{__('Remind At')}}</th>
                                                ...
                                                <tbody class="list">
                                                    @forelse ($deal->reminders as $reminder)
                                                        <tr>
                                                            <td>{{ $reminder->title }}</td>
                                                            <td>{{ company_datetime_formate($reminder->remind_at) }}</td>
                                                            <td>{{ $reminder->user->name ?? '-' }}</td>
                                                            <td>{{ __(ucfirst($reminder->type)) }}</td>
                                                            <td class="Action text-end">
                                                                <span>
                                                                    <div class="action-btn me-2">
                                                                        <a data-size="md"
                                                                            data-url="{{ route('deals.reminders.edit', [$deal->id, $reminder->id]) }}"
                                                                            data-ajax-popup="true"
                                                                    ...
                                                                    <div class="action-btn">
                                                                        {!! Form::open([
                                                                            'method' => 'DELETE',
                                                                            'route' => ['deals.reminders.destroy', $deal->id, $reminder->id],
                                                                            'id' => 'delete-reminder-form-' . $reminder->id,
                                                                        ]) !!}
                                                                        <a href="#!"
                                                                            class="mx-3 btn btn-sm align-items-center show_confirm bg-danger"
                                                                            data-bs-toggle="tooltip"
