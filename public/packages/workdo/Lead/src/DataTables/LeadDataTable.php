...
class LeadDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        $dataTable = (new EloquentDataTable($query))
            ...
            ->editColumn('phone', function (Lead $lead) {
                return LeadUtility::getFieldDisplay($lead->id, 'phone', $lead->phone);
            })
    ...
    public function query(Lead $model)
    {
        ...
        if ($user->type != 'company' && $user->visibility_level != 'all') {
            ...
            $query->whereHas('users', function($q) use ($accessibleUserIds) {
                $q->whereIn('users.id', $accessibleUserIds);
            });
        }
        ...
        if ($request->has('responsible_person') && !empty($request->responsible_person)) {
            $query->whereHas('users', function($q) use ($request) {
                $q->whereIn('users.id', (array)$request->responsible_person);
            });
        }
