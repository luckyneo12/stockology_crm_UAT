    ...
    $.get(baseUrl + '/notifications/latest-unread', function (notification) {
        if (notification && notification.id) {
            ...
            if (lastNotifiedId != notification.id) {
                localStorage.setItem('last_notified_notification_id', notification.id);
                ...
                if (notification.type == 'lead_assigned') {
                    ...
                    icon = 'warning'; // Orange alert
                } else if (notification.type == 'new_comment') {
                    title = 'New Comment';
                    icon = 'question'; // Purple question mark (closest to chat bubble)
                } else if (notification.type == 'reminder') {
                    title = 'Reminder';
                    icon = 'error'; // Red error (closest to urgent bell)
                } else if (notification.type == 'kyc_comment') {
                    title = 'KYC Update';
                    icon = 'info';
                }
                ...
                // Fallback for KYC comments which might structure data differently
                if (notification.type == 'kyc_comment' && data.comment) {
                    body = data.created_by_name + ' commented: ' + data.comment;
                }
...
function common_bind(selector = "body") {
    ...
    var $choices_elements = $(selector).find("select.choices, input.choices");
    if (selector === "body") { $choices_elements = $("select.choices, input.choices"); }

    if ($choices_elements.length > 0) {
        $choices_elements.each(function (index, element) {
            var searchEnabled = $(element).attr('searchEnabled') == 'true' ? true : false;

            if (element.choicesInstance) {
                element.choicesInstance.destroy();
            }
            element.choicesInstance = new Choices(
                element, {
                loadingText: 'Loading...',
...
function choices(id = null) {
    var target = id ? "#" + id : "select.choices, input.choices";
    if ($(target).length > 0) {
