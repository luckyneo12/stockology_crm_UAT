 ...
 *-------------------------------------------------------------
 */
if (window.chatifyCodeLoaded) {
    console.warn('Chatify code already loaded, skipping execution.');
} else {
    window.chatifyCodeLoaded = true;
 ...
 `;
        ...
        if (messenger != 0) {
            ...
            $.ajax({
                ...
                success: (data) => {
                    ...
                    $('.messenger-infoView .info-email span').text(data.fetch.email);
                    $('.messenger-infoView .info-phone span').text(data.fetch.mobile_no || '');
                    $('.messenger-infoView .info-role span').text(data.fetch.type || '');
    ...
    */
    ...
    channel.bind('messaging', function (data) {
        ...
        } else if (data.to_id == auth_id && data.from_id != messenger.split('_')[1]) {
            // Message from someone else (not in current conversation)
            // Show floating notification
            if (typeof toastrs === 'function') {
                // Extract sender name from data if available, otherwise use generic message
                const senderName = data.from_name || 'New Message';
                const messagePreview = data.message_text || 'You have a new message';
                toastrs('Message', senderName + ': ' + messagePreview, 'info');
            }
        }
    });
    ...
    */
    $(document).ready(function () {
        ...
        $('.messenger-search').on('focus', function () {
            // Only show search tab if we want global search, otherwise keep current tab
            // For now, we want to filter the current list, so we disable the auto-switch
            // $('.messenger-tab').hide();
        ...
        $('.messenger-search').on('keyup', function (e) {
            ...
            // Filter users list
            if ($('.messenger-listView-tabs a[data-view="users"]').hasClass('active-tab')) {
                var $usersContainer = $('.messenger-tab[data-view="users"]');

                // Filter both favorites and contact list items
                $usersContainer.find('.messenger-list-item').each(function () {
                    var $p = $(this).find('td > p[data-id]');
                    var name = $p.attr('data-name') ? $p.attr('data-name').toLowerCase() : $p.text().toLowerCase();

                    // check if the item is a favorite or contact
                ...
                // Should also filter favorites specifically if they have a different structure
                $usersContainer.find('.favorite-list-item').each(function () {
                    var $p = $(this).find('p');
                    var name = $p.attr('data-name') ? $p.attr('data-name').toLowerCase() : $p.text().toLowerCase();
            ...
            // Keep original server-side search if Enter is pressed or explicitly requested?
            // For now, let's keep the filter as the primary action. 
            // If the user wants global search, we might need a specific button or "Enter" key behavior.

            if ($.trim($(this).val()).length > 0) {
                // Check if we are in search tab, then do server search
                if ($('.messenger-listView-tabs a[data-view="search"]').hasClass('active-tab')) {
                    messengerSearch($(this).val());
                }
            } else {
                // Reset if empty
                $('.messenger-tab[data-view="users"]').find('.messenger-list-item').show();
                $('.messenger-tab[data-view="users"]').find('.favorite-list-item').show();
            }
        });
