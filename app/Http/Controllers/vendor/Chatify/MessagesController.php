...
class MessagesController extends Controller
{
    ...
    public function __construct()
    {
        ...
        if (isset($admin_settings['PUSHER_APP_KEY']))
            config(['chatify.pusher.key' => $admin_settings['PUSHER_APP_KEY']]);
        if (isset($admin_settings['PUSHER_APP_SECRET']))
            config(['chatify.pusher.secret' => $admin_settings['PUSHER_APP_SECRET']]);
        if (isset($admin_settings['PUSHER_APP_ID']))
            config(['chatify.pusher.app_id' => $admin_settings['PUSHER_APP_ID']]);
        if (isset($admin_settings['PUSHER_APP_CLUSTER']))
            config(['chatify.pusher.options.cluster' => $admin_settings['PUSHER_APP_CLUSTER']]);
    }
        ...
        if (!$error->status) {
            ...
            try {
                Chatify::push(
                    ...
                    'message_text' => strip_tags(html_entity_decode($request['message'])),
                ]
                );
            }
            catch (\Exception $e) {
                \Log::error("Chatify Pusher Error: " . $e->getMessage());
            }
        }
