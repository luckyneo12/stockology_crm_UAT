...
if (!function_exists('sip_link')) {
    function sip_link($phone_number)
    {
        $company_settings = getCompanyAllSetting();
        $sip_domain = !empty($company_settings['sip_server_domain']) ? $company_settings['sip_server_domain'] : '';
        if (empty($sip_domain)) {
            return 'tel:' . $phone_number;
        }
        return 'sip:' . $phone_number . '@' . $sip_domain;
    }
}
