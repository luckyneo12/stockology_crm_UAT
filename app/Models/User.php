    ...
    public static function CompanySetting($id = null, $workspace_id = null)
    {
        ...
        if (!empty($id)) {
            ...
            $company_setting = [
                ...
                "title_text" => !empty($admin_settings['title_text']) ? $admin_settings['title_text'] : "stockology_CRM",
                "footer_text" => !empty($admin_settings['footer_text']) ? $admin_settings['footer_text'] : "Copyright © stockology_CRM",
    ...
    public function getAccessibleUserIds()
    {
        ...
        if (module_is_active('Hrm')) {
            ...
            if ($employee) {
                ...
                if ($this->visibility_level == 'all' || $this->visibility_level == 'team' || empty($this->visibility_level)) {
                    $subordinates = $employee->allSubordinateIds();
                    if (!empty($subordinates)) {
                        $user_ids = array_merge($user_ids, $subordinates);
                    }

                    // DEFAULT: Show colleagues in the same department
                    if (empty($this->visibility_level) || $this->visibility_level == 'department') {
                        $deptUsers = \Workdo\Hrm\Entities\Employee::where('department_id', $employee->department_id)
        ...
        if (!empty($this->accessible_departments)) {
            $deptUserIds = \Workdo\Hrm\Entities\Employee::whereIn('department_id', $this->accessible_departments)
                ...
                ->pluck('user_id')
                ->toArray();
            $user_ids = array_merge($user_ids, $deptUserIds);
        }

        // 4. Accessible Users (Cross-User Visibility)
