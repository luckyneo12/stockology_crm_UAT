...
class DealController extends Controller
{
    ...
    public function create()
    {
        if (Auth::user()->isAbleTo('deal create')) {
            ...
            if (module_is_active('CustomField')) {
                ...
                if($stage){
                    $stageCustomFields = StageCustomField::where('stage_id', $stage->id)->where('entity_type', 'deal')->pluck('custom_field_id')->toArray();
    ...
    public function store(Request $request)
    {
        ...
        if ($usr->isAbleTo('deal create')) {
            ...
            if($request->has('customField')){
                 ...
                 if($pipelineId){
                     $stage = DealStage::where('pipeline_id', '=', $pipelineId)->where('workspace_id', $getActiveWorkSpace)->first();
                     if($stage){
                         $requiredFields = StageCustomField::where('stage_id', $stage->id)->where('entity_type', 'deal')->where('is_required', 1)->pluck('custom_field_id')->toArray();
    ...
    public function show(Deal $deal)
    {
        if (Auth::user()->isAbleTo('deal show')) {
            if ($deal->is_active) {
                ...
                $tasks = $deal->tasks()->whereIn('user_id', $accessibleUserIds)->get();
                $reminders = $deal->getFilteredReminders();
                ...
                $todayRemindersCount = $deal->getTodayRemindersCount();

                return view('lead::deals.show', compact('deal', 'transdate', 'calenderTasks', 'permission', 'customFields', 'tasks', 'reminders', 'overdueTasksCount', 'todayRemindersCount'));
            } else {
                return redirect()->back()->with('error', __('Permission Denied.'));
            }
        } else {
        ...
        if (Auth::user()->isAbleTo('deal edit')) {
            ...
            if ($deal->created_by == $creatorId) {
                ...
                if (module_is_active('CustomField')) {
                    ...
                    // Filter by Stage Visibility
                    $stageCustomFields = StageCustomField::where('stage_id', $deal->stage_id)->where('entity_type', 'deal')->pluck('custom_field_id')->toArray();
                    if(!empty($stageCustomFields)){
                        $customFields = $customFields->filter(function($field) use ($stageCustomFields){
                            return in_array($field->id, $stageCustomFields);
                        });
    ...
    public function update(Request $request, Deal $deal)
    {
        if (Auth::user()->isAbleTo('deal edit')) {
            ...
            if ($deal->created_by == $creatorId && $deal->workspace_id == $getActiveWorkSpace) {
                ...
                if($request->has('customField')){
                     $requiredFields = StageCustomField::where('stage_id', $request->stage_id)->where('entity_type', 'deal')->where('is_required', 1)->pluck('custom_field_id')->toArray();
                     foreach($request->customField as $id => $value){
                         if(in_array($id, $requiredFields) && empty($value)){
                             $validator->after(function ($validator) use($id) {
                                 $validator->errors()->add('customField.'.$id, __('This custom field is required.'));
                ...
                if($deal->stage_id != $request->stage_id){
                     PipelineStageAutomation::run($deal, $request->stage_id);
                }
        ...
        try {
            ...
            if ($usr->isAbleTo('deal move')) {
                ...
                if ($deal->stage_id != $post['stage_id']) {
                    ...
                    // Automation Logic (Centralized)
                    PipelineStageAutomation::run($deal, $post['stage_id']);
    ...
    public function taskCreate($id)
    {
        if (Auth::user()->isAbleTo('deal task create')) {
            ...
            if ($deal && $deal->isAccessible()) {
                ...
                return view('lead::deals.tasks', compact('deal', 'priorities', 'status', 'users'));
            } else {
                return response()->json(
                    [
                        'is_success' => false,
    ...
    public function taskStore($id, Request $request)
    {
        ...
        if ($usr->isAbleTo('deal task create')) {
            ...
            if ($deal && $deal->isAccessible()) {
                ...
                if (!empty(company_setting('Deal Task Created')) && company_setting('Deal Task Created')  == true) {
                    $deal_users = $deal->users->pluck('id')->toArray();
                    $clients    = ClientDeal::select('client_id')->where('deal_id', '=', $deal->id)->get()->pluck('client_id')->toArray();
                    $usrs       = User::whereIN('id', array_merge($deal_users, $clients))->get()->pluck('email', 'id')->toArray();
                    ...
                    // Send Email
                    $resp = EmailTemplate::sendEmailTemplate('Deal Task Created', $usrs, $tArr);
                }
    ...
    public function taskEdit($id, $task_id)
    {
        if (Auth::user()->isAbleTo('deal task edit')) {
            ...
            if ($deal && $deal->isAccessible()) {
                ...
                return view('lead::deals.tasks', compact('task', 'deal', 'priorities', 'status', 'users'));
            } else {
                return response()->json(
                    [
                        'is_success' => false,
    ...
    public function taskDestroy($id, $task_id)
    {
        if (Auth::user()->isAbleTo('deal task delete')) {
            ...
            if ($deal && $deal->isAccessible()) {
                ...
                event(new DestroyDealTask($task));
    ...
    {
        if (Auth::user()->isAbleTo('deal manage')) {
            $deal = Deal::find($id);
            if ($deal && $deal->isAccessible()) {
                $types = Reminder::$types;
                $users = User::where('created_by', '=', creatorId())->where('workspace_id', getActiveWorkSpace())->get()->pluck('name', 'id');
                return view('lead::deals.reminders', compact('deal', 'types', 'users'));
            }
        }
        return response()->json(['error' => __('Permission Denied.')], 401);
    }
    ...
    public function reminderEdit($id, $reminder_id)
    {
        if (Auth::user()->isAbleTo('deal manage')) {
            ...
            if ($deal && $deal->isAccessible()) {
                ...
                $types = Reminder::$types;
                $users = User::where('created_by', '=', creatorId())->where('workspace_id', getActiveWorkSpace())->get()->pluck('name', 'id');
                return view('lead::deals.reminders', compact('deal', 'reminder', 'types', 'users'));
            }
        }
        return response()->json(['error' => __('Permission Denied.')], 401);
    }
    ...
    public function fileImport(Request $request)
    {
        if (Auth::user()->isAbleTo('deal import')) {
            ...
            if ($request->hasFile('file') && $request->file('file')->getClientOriginalName() != '') {
                ...
                if ($extension == 'csv') {
                    $file_data = fopen($file->getRealPath(), 'r');
