...
class LeadUtility extends Model
{
    ...
    public static function canSeeField($leadId, $fieldName)
    {
        ...
        if($user->isAbleTo('lead show protected data')) return true;

        foreach ($rules as $rule) {
            // Check Stage Restriction
            if ($rule->pipeline_id && $rule->pipeline_id != $lead->pipeline_id) continue;
            if ($rule->stage_id && $rule->stage_id != $lead->stage_id) continue;
    ...
    public static function getFieldDisplay($lead, $fieldName, $originalValue)
    {
        ...
        if (!$leadInstance) return $originalValue;

        foreach ($rules as $rule) {
             // Context Check
             if ($rule->pipeline_id && $rule->pipeline_id != $leadInstance->pipeline_id) continue;
