...
class LeadDataTable extends DataTable
{
    ...
    {
        $rowColumn = ['batch', 'name', 'subject', 'email', 'phone', 'stage_id', 'tasks', 'reminders', 'created_at', 'user_id'];
        $dataTable = (new EloquentDataTable($query))
            ...
            return LeadUtility::getFieldDisplay($lead->id, 'name', $lead->name);
        })
            ->editColumn('subject', function (Lead $lead) {
            return LeadUtility::getFieldDisplay($lead->id, 'subject', $lead->subject);
        })
            ->editColumn('email', function (Lead $lead) {
            return LeadUtility::getFieldDisplay($lead->id, 'email', $lead->email);
        })
            ->editColumn('phone', function (Lead $lead) {
            $phone = LeadUtility::getFieldDisplay($lead->id, 'phone', $lead->phone);
            if (!empty($lead->phone)) {
            ...
            ->addColumn('reminders', function (Lead $lead) {
    ...
    public function query(Lead $model)
    {
        ...
        if (($request->has('modified_start_date') && !empty($request->modified_start_date)) || ($request->has('modified_end_date') && !empty($request->modified_end_date))) {
            $query->where(function ($q) use ($request) {
                ...
                $q->where(function ($sub) use ($m_start, $m_end) {
                        if (!empty($m_start)) {
                    ...
                    foreach ($relations as $rel) {
                        $q->orWhereHas($rel, function ($sub) use ($m_start, $m_end) {
                                if (!empty($m_start)) {
        ...
        if ($request->has('department_id') && !empty($request->department_id)) {
            ...
            $query->where(function ($q) use ($deptUserIds) {
                $q->whereIn('leads.user_id', $deptUserIds)
                    ->orWhereHas('users', function ($subQ) use ($deptUserIds) {
        ...
        if ($request->has('designation_id') && !empty($request->designation_id)) {
            ...
            $query->where(function ($q) use ($desigUserIds) {
                $q->whereIn('leads.user_id', $desigUserIds)
                    ->orWhereHas('users', function ($subQ) use ($desigUserIds) {
        ...
        if ($request->has('duplicates') && $request->duplicates == 1) {
            ...
            $query->where(function ($q) use ($wsId) {
                $q->where(function ($inner) use ($wsId) {
                        $inner->whereNotNull('leads.email')->where('leads.email', '!=', '')->whereIn('leads.email', function ($sub) use ($wsId) {
                                $sub->select('email')->from('leads')->where('workspace_id', $wsId)->whereNotNull('email')->where('email', '!=', '')->groupBy('email')->havingRaw('COUNT(email) > 1');
                            }
                            );
                        }
                        )
                            ->orWhere(function ($inner) use ($wsId) {
                    $inner->whereNotNull('leads.phone')->where('leads.phone', '!=', '')->whereIn('leads.phone', function ($sub) use ($wsId) {
                            ...
                            ->orWhere(function ($inner) use ($wsId) {
                    $inner->whereNotNull('leads.name')->where('leads.name', '!=', '')->whereIn('leads.name', function ($sub) use ($wsId) {
