{{ Form::open(array('url' => 'webhook-endpoints')) }}
<div class="modal-body">
    <div class="row">
        <div class="form-group col-12">
            {{ Form::label('name', __('Endpoint Name'),['class'=>'form-label']) }}
            {{ Form::text('name', '', array('class' => 'form-control','required'=>'required', 'placeholder'=>__('e.g. Facebook Lead Ads'))) }}
        </div>
        <div class="form-group col-12">
        ...
        <div class="form-group col-12">
            ...
            {!! Form::select('edit_permissions[]', $users, null, array('class' => 'form-control choices', 'multiple' => 'multiple', 'id' => 'edit_permissions')) !!}
        </div>

        <div class="form-group col-12">
            {{ Form::label('view_permissions', __('View Permissions (Users who can see & transfer incoming data)'),['class'=>'form-label']) }}
            {!! Form::select('view_permissions[]', $users, null, array('class' => 'form-control choices', 'multiple' => 'multiple', 'id' => 'view_permissions')) !!}
        </div>
                ...
                <table class="table table-bordered" id="mapping_table">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>{{ Form::text('field_mapping[name]', 'name', array('class' => 'form-control', 'required' => 'required', 'placeholder' => __('e.g. customer_name'))) }}</td>
                            <td class="text-center">{{ Form::checkbox('field_mapping[in_form][name]', 1, true) }}</td>
                            <td></td>
                        </tr>
                        <tr>
                            <td>{{__('Email')}}</td>
                            <td>{{ Form::text('field_mapping[email]', 'email', array('class' => 'form-control', 'placeholder' => __('e.g. customer_email'))) }}</td>
                            <td class="text-center">{{ Form::checkbox('field_mapping[in_form][email]', 1, true) }}</td>
                            <td></td>
                        </tr>
                        <tr>
                            <td>{{__('Phone')}}</td>
                            <td>{{ Form::text('field_mapping[phone]', 'phone', array('class' => 'form-control', 'placeholder' => __('e.g. mobile_number'))) }}</td>
                            <td class="text-center">{{ Form::checkbox('field_mapping[in_form][phone]', 1, true) }}</td>
                            <td></td>
                        </tr>
                        <tr>
                            <td>{{__('Subject')}}</td>
                            <td>{{ Form::text('field_mapping[subject]', 'subject', array('class' => 'form-control', 'placeholder' => __('e.g. interest'))) }}</td>
                            <td class="text-center">{{ Form::checkbox('field_mapping[in_form][subject]', 1, true) }}</td>
                            <td></td>
                        </tr>
                        @foreach($customFields as $field)
                            <tr>
                                <td>{{ $field->name }} ({{__('Custom')}})</td>
                                <td>{{ Form::text('field_mapping[custom]['.$field->id.']', '', array('class' => 'form-control', 'placeholder' => __('JSON Key'))) }}</td>
                                <td class="text-center">{{ Form::checkbox('field_mapping[in_form][custom]['.$field->id.']', 1, false) }}</td>
                                <td></td>
                            </tr>
                        @endforeach
                    </tbody>
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(data, function(key, data) {
                    stage_select.append('<option value="' + key + '">' + data + '</option>');
                });
            }
        });
    }
