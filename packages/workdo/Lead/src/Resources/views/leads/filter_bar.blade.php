...
<style>
    ...
    .filter-card:hover, .filter-card:focus-within {
...
<div class="row align-items-center mb-3 g-2">
    <div class="col-xl-4 col-lg-5 col-md-7 col-sm-12">
        <div class="input-group input-group shadow-sm rounded position-relative">
            ...
            <input type="text" id="lead_search" class="form-control border-start-0 ps-0" placeholder="{{ __('Quick search...') }}" value="{{ request('search') }}">
            <button class="input-group-text bg-white border-start-0" type="button" data-bs-toggle="modal" data-bs-target="#searchSettingsModal" title="{{ __('Search Settings') }}">
        ...
        <button type="button" class="btn btn-primary btn-icon shadow-sm position-relative" data-bs-toggle="modal" data-bs-target="#leadFilterModal" id="advancedFilterBtn">
            ...
            @php
                ...
                foreach($filterKeys as $key) {
                    ...
                    elseif(request()->has($key . '[]') && !empty(request($key . '[]'))) $activeFilterCount++;
                }
            @endphp
                ...
                <div class="row g-4">
                            ...
                            <div class="col-md-6 text-start mt-3">
                                <div class="form-group mb-0">
                                    ...
                                    <div class="input-group shadow-sm border rounded overflow-hidden">
                                        <input type="text" class="form-control border-0 flatpickr-filter" name="start_date" id="modal_start_date" value="{{ request('start_date') }}" placeholder="YYYY-MM-DD">
                                        <span class="input-group-text bg-white border-0"><i class="ti ti-arrow-right text-muted"></i></span>
                                        <input type="text" class="form-control border-0 flatpickr-filter" name="end_date" id="modal_end_date" value="{{ request('end_date') }}" placeholder="YYYY-MM-DD">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6 text-start mt-3">
                                <div class="form-group mb-0">
                                    <label class="form-label fw-bold text-dark mb-1">{{ __('Date Modified Range') }}</label>
                                    <div class="input-group shadow-sm border rounded overflow-hidden">
                                        <input type="text" class="form-control border-0 flatpickr-filter" name="modified_start_date" id="modal_modified_start_date" value="{{ request('modified_start_date') }}" placeholder="YYYY-MM-DD">
                                        <span class="input-group-text bg-white border-0"><i class="ti ti-arrow-right text-muted"></i></span>
                                        <input type="text" class="form-control border-0 flatpickr-filter" name="modified_end_date" id="modal_modified_end_date" value="{{ request('modified_end_date') }}" placeholder="YYYY-MM-DD">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-12">
                        <div class="p-3 bg-light rounded-3 border border-light filter-card">
                            <div class="d-flex align-items-center mb-3">
                                <span class="badge bg-primary p-2 rounded-circle me-2"><i class="ti ti-building text-white"></i></span>
                                <h6 class="text-dark fw-bold mb-0 filter-section-title">{{ __('Department') }}</h6>
                            </div>
                            <div class="row">
                                <div class="col-12">
                                    <div class="form-group mb-0">
                                        {!! Form::select('department_id[]', $departments, request('department_id'), ['class' => 'form-control choices-js-filter', 'multiple' => 'multiple', 'id' => 'modal_department_id', 'data-placeholder' => __('Select Department')]) !!}
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-12">
                        <div class="p-3 bg-light rounded-3 border border-light filter-card">
                            <div class="d-flex align-items-center mb-3">
                                <span class="badge bg-info p-2 rounded-circle me-2"><i class="ti ti-sitemap text-white"></i></span>
                                <h6 class="text-dark fw-bold mb-0 filter-section-title">{{ __('Team') }}</h6>
                            </div>
                            <div class="row">
                                <div class="col-12">
                                    <div class="form-group mb-0">
                                        {!! Form::select('designation_id[]', $designations, request('designation_id'), ['class' => 'form-control choices-js-filter', 'multiple' => 'multiple', 'id' => 'modal_designation_id', 'data-placeholder' => __('Select Team')]) !!}
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="col-12">
                        <div class="p-3 bg-light rounded-3 border border-light filter-card">
                            <div class="d-flex align-items-center mb-3">
                                <span class="badge bg-success p-2 rounded-circle me-2"><i class="ti ti-tag text-white"></i></span>
                                <h6 class="text-dark fw-bold mb-0 filter-section-title">{{ __('Classification') }}</h6>
                            </div>
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <div class="form-group mb-0">
                                        <label class="form-label fw-bold text-muted small mb-1">{{ __('Source') }}</label>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content shadow-lg border-0" style="border-radius: 15px;">
            ...
            <div class="modal-body pt-3">
                ...
                <form id="searchSettingsForm">
                    ...
                    <div class="list-group list-group-flush">
                        ...
                        <label class="list-group-item border-0 d-flex justify-content-between align-items-center px-0 py-2 cursor-pointer">
                            <span class="text-dark fw-medium">{{ $label }}</span>
                            <div class="form-check form-switch m-0">
                                <input class="form-check-input" type="checkbox" name="search_fields[]" value="{{ $key }}" {{ in_array($key, $userSettings) ? 'checked' : '' }}>
                            </div>
                        </label>
                        @endforeach
                        ...
                        @if($customFields->count() > 0)
                            ...
                            <label class="list-group-item border-0 d-flex justify-content-between align-items-center px-0 py-2 cursor-pointer">
                                ...
                                <div class="form-check form-switch m-0">
                                    <input class="form-check-input" type="checkbox" name="search_fields[]" value="custom_{{ $field->id }}" {{ in_array('custom_' . $field->id, $userSettings) ? 'checked' : '' }}>
                                </div>
                            </label>
                            @endforeach
                        @endif
            ...
            <div class="modal-footer border-0 pt-0">
                <button type="button" class="btn btn-light rounded-pill w-100 mb-2" data-bs-dismiss="modal">{{ __('Cancel') }}</button>
                <button type="button" class="btn btn-primary rounded-pill w-100" id="saveSearchSettingsBtn">{{ __('Save Configuration') }}</button>
            </div>
        </div>
    </div>
</div>
