        ...
        if ($project) {
            ...
            if ($stage) {
                ...
                if ($request->get('assign_to')) {
                    foreach ($request->get('assign_to') as $usr) {
                        // Create Notification
                        \App\Models\UserNotification::create([
                            'user_id' => $usr,
                            'type' => 'task_assigned',
                            'data' => json_encode(['message' => 'New Task Assigned: ' . $task->title, 'id' => $task->id]),
                            'is_read' => 0,
                            'workspace_id' => getActiveWorkSpace()
                        ]);
                    }
        ...
        foreach ($assignees as $userId) {
            if ($userId != \Auth::user()->id) {
                \App\Models\UserNotification::create([
                    'user_id' => $userId,
                    'type' => 'new_comment',
                    'data' => json_encode(['message' => 'New Comment on Task: ' . $task->title, 'id' => $task->id]),
                    'is_read' => 0,
                    'workspace_id' => getActiveWorkSpace()
                ]);
            }
